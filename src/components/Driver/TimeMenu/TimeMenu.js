...
import {sendTimeToDriver} from "../../../fetch/fetch";
    ...
    return(
        <div className={props.isOpen ? "message-menu-bg message-menu-bg--open" : "message-menu-bg"} onClick={props.closeMenu}>
            <div className="message-menu">
                ...
                <button
                    onClick={()=>{
                        sendTimeToDriver(0,props.token).then((data)=>{
                            console.log(data);
                            props.closeMenu();
                        });
                    }}
                ...
                <button
                    onClick={()=>{
                        sendTimeToDriver(10,props.token).then(()=>{
                            props.closeMenu();
                        });
                    }}
                >5 мин</button>
                <button
                    onClick={()=>{
                        sendTimeToDriver(15,props.token).then(()=>{
                            props.closeMenu();
                        });
                    }}
                >15 мин</button>
                <button
                    onClick={()=>{
                        sendTimeToDriver(20,props.token).then(()=>{
                            props.closeMenu();
                        });
                    }}
                >20 мин</button>
