...
import {addressAutocomplete, getStreet, HTTP_STATUS_USER_UNAUTHORIZED} from '../../../fetch/fetch';
...
export default class OrderInput extends Component{
    ...
    promiseOptions(inputValue){
        ...
        return addressAutocomplete(inputValue).then((data)=>{
    ...
    render() {
        return (
            <AsyncSelect   backspaceRemovesValue={false} isClearable={true} classPrefix="react-select" placeholder={'Выберите точку'}  value={this.props.defaultValue} onChange={(optionSelected,action) => this.props.onChangeCallback(optionSelected,action, this.props.id)} class="order-input" cacheOptions defaultOptions={this.constOptions} loadOptions={this.getOptions.bind(this)} />
        );
    }
}